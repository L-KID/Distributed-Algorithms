...
//		int port = Integer.parseInt(args[0]) + 1098;
        System.setProperty("java.rmi.server.hostname", "localhost");
		     ...
		     registry.bind("rmi://localhost:1097/RemoteServer2", p);
	...
	RemoteInterf[] RMI_IDS = new RemoteInterf[numProcess]; 
		...
		BufferedReader buffer = new BufferedReader(new FileReader("messages2.txt"));
		String line = "";
		String[] split_line;
		while ((line = buffer.readLine()) != null) {
			
			split_line = line.split(" ");
			//((RemoteInterf) Naming.lookup("rmi://localhost:1098/RemoteServer" + procID)).addMessage(split_line[0], Integer.parseInt(split_line[1]));
